...
6. `runtime/data/cases.json` 只包含十四个已批准第一方案例，且 Plugin 内不含 `.png`、`.jpg`、`.webp` 等 Gallery 栅格图。
...
6. `runtime/data/cases.json` contains only the fourteen approved first-party examples, and the Plugin contains no `.png`, `.jpg`, or `.webp` Gallery rasters.
