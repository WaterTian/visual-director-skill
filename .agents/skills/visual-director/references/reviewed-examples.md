...
uv run python scripts/search-cases.py --id 12 --full-prompt
uv run python scripts/search-cases.py --id 13 --full-prompt
...
Highly specialized examples are considered only when the Brief states their distinctive intent, so a generic portrait or studio request is not pulled toward a narrowly scoped workflow. / 高度专门化的案例仅在 Brief 明确包含其特有意图时才会参与排序，普通人像或影棚请求不会被窄范围工作流错误牵引。
